...
-- 컬럼명 생략시 테이블구조 순서대로 자동 기입 (되도록 기입해 두는걸 권장)
INSERT INTO goods
    (goods_name, id, reg_date, price)
VALUES
...
-- 조건 없이 delete하면 전체삭제됨 (다만 이 문법은 복구 가능)
DELETE FROM goods;
TRUNCATE TABLE goods; -- 복구 불가, 테이블 생성 초기로 돌아가는 것(DDL)
DROP TABLE goods; -- 복구불가 + 테이블도 날라감
...
-- 모든 컬럼 조회 (수정 및 유지보수 불편해서 사용을 권장하진 않음)
SELECT
    *
FROM tb_certi;
