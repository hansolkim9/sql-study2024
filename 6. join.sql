...
z    D.dept_cd,
    D.dept_nm
FROM tb_emp E, tb_dept D -- FROM에 테이블 나열하는건 오라클만 가능
...
WHERE E.emp_no = ec.emp_no AND EC.certi_cd = C.certi_cd
;

-- 사원의 사원번호와 사원명과 취득한 자격증명, 부서까지 조회
SELECT
    E.emp_no 사번,
    E.emp_nm 사원명,
    D.dept_nm 부서명,
...
SELECT
    ...
    D.dept_nm
FROM tb_emp E
JOIN tb_dept D -- (INNER 생략) JOIN ~ ON ~ : 표준어
...
-- 1980년대생 사원들의 사번, 사원명, 부서명, 자격증명, 취득일자 조회
SELECT
...
-- INNER 조인은 두 테이블 간에 연관 데이터가 있는 경우에만 사용
